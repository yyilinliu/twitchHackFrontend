...
let copyIconsToResSubfoldersAsync = (() => {
  var _ref5 = _asyncToGenerator(function* (resDirPath, folderPrefix, folderSuffix, fileName, iconUrl, isLocalUrl) {
    return Promise.all(imageKeys.map((() => {
      var _ref6 = _asyncToGenerator(function* (key) {
        try {
          const dirPath = _path.default.join(resDirPath, `${folderPrefix}${key}${folderSuffix}`);
          (_fsExtra || _load_fsExtra()).default.accessSync(dirPath, (_fsExtra || _load_fsExtra()).default.constants.F_OK);
          if (isLocalUrl) {
            return (_fsExtra || _load_fsExtra()).default.copy(iconUrl, _path.default.join(dirPath, fileName));
          }
          return saveUrlToPathAsync(iconUrl, _path.default.join(dirPath, fileName));
        } catch (e) {
  ...
  return function copyIconsToResSubfoldersAsync(_x6, _x7, _x8, _x9, _x10, _x11) {
    return _ref5.apply(this, arguments);
  };
})();

let regexFileInResSubfoldersAsync = (() => {
  var _ref7 = _asyncToGenerator(function* (oldText, newText, resDirPath, folderPrefix, folderSuffix, fileName) {
    return Promise.all(imageKeys.map((() => {
      var _ref8 = _asyncToGenerator(function* (key) {
        return regexFileAsync(oldText, newText, _path.default.join(resDirPath, `${folderPrefix}${key}${folderSuffix}`, fileName));
      });
  ...
  return function regexFileInResSubfoldersAsync(_x13, _x14, _x15, _x16, _x17, _x18) {
...
let runShellAppModificationsAsync = exports.runShellAppModificationsAsync = (() => {
  var _ref9 = _asyncToGenerator(function* (context, isDetached = false) {
    ...
    if (iconUrl || iconForegroundUrl) {
      if (iconUrl) {
        ...
        yield copyIconsToResSubfoldersAsync(_path.default.join(shellPath, 'app', 'src', 'main', 'res'), 'mipmap-', '', 'ic_launcher.png', iconUrl, isDetached);
      }
      ...
      if (iconForegroundUrl) {
        ...
        yield copyIconsToResSubfoldersAsync(_path.default.join(shellPath, 'app', 'src', 'main', 'res'), 'mipmap-', '-v26', 'ic_foreground.png', iconForegroundUrl, isDetached);
      } else {
    ...
    if (iconBackgroundUrl) {
      yield copyIconsToResSubfoldersAsync(_path.default.join(shellPath, 'app', 'src', 'main', 'res'), 'mipmap-', '-v26', 'ic_background.png', iconBackgroundUrl, isDetached);

      yield regexFileInResSubfoldersAsync('@color/iconBackground', '@mipmap/ic_background', _path.default.join(shellPath, 'app', 'src', 'main', 'res'), 'mipmap-', '-v26', 'ic_launcher.xml');
    } else if (iconBackgroundColor) {
      yield regexFileAsync('"iconBackground">#FFFFFF', `"iconBackground">${iconBackgroundColor}`, _path.default.join(shellPath, 'app', 'src', 'main', 'res', 'values', 'colors.xml'));
    }
    ...
    if (notificationIconUrl) {
      ...
      yield copyIconsToResSubfoldersAsync(_path.default.join(shellPath, 'app', 'src', 'main', 'res'), 'drawable-', '', 'shell_notification_icon.png', notificationIconUrl, isDetached);
    }
  ...
  return function runShellAppModificationsAsync(_x20) {
...
let buildShellAppAsync = (() => {
  ...
  return function buildShellAppAsync(_x22) {
